...
usr = params['usr']  # the user (simulator) i

# <editor-fold desc="Set Seeds">
seed = 2
...
# split goal set
split_fold = params.get('split_fold', 5)  # the number of folders to split the user goal
...
# <editor-fold desc="Path to Act_set and Slot_set">
...
# <editor-fold desc="Parameters for Agent">
...
# <editor-fold desc="Select Agents">
########## Select Agents ##########
if agt == 0:
...
# <editor-fold desc="Parameters for User Simulators">
...
# <editor-fold desc="Select User(Simulator)">
########### Select User(Simulator) ###########
if usr == 0:  # real user
...
# <editor-fold desc="Save Best Model and Performance Record">
...
def save_model(path, agt, success_rate, agent, best_epoch, cur_epoch):
    ...
    try:
        ...
        print('saved model in %s'.format(filepath, ))
    except Exception as e:
        print('Error: Writing model fails: %s'.format(filepath, ))
        print(e)
...
# <editor-fold desc="Running settings">
########## Running settings ##########
...
# <editor-fold desc="Warm_Start Simulation (by Rule Policy)">
def warm_start_simulation():
        ...
        for episode in range(warm_start_epochs):
            ...
            while (not episode_over):
                ...
                if episode_over:
                    if reward > 0:
                        successes += 1
                        print("warm_start simulation episode %s: Success".format(episode))
                    else:
                        print("warm_start simulation episode %s: Fail".format(episode))
    ...
    print("Warm_Start %s epochs, success rate %s, ave reward %s, ave turns %s".format(
        episode + 1, res['success_rate'], res['ave_reward'], res['ave_turns']))
    print("Current experience replay buffer size %s".format(len(agent.experience_replay_pool)))
    ...
    for episode in range(simulation_epoch_size):
        ...
        while (not episode_over):
            ...
            if episode_over:
                if reward > 0:
                    successes += 1
                    print("simulation episode %s: Success".format(episode))
                else:
                    print("simulation episode %s: Fail".format(episode))
                cumulative_turns += dialog_manager.state_tracker.turn_count

    res['success_rate'] = float(successes) / simulation_epoch_size
    res['ave_reward'] = float(cumulative_reward) / simulation_epoch_size
    res['ave_turns'] = float(cumulative_turns) / simulation_epoch_size
    print("simulation success rate %s, ave reward %s, ave turns %s".format(res['success_rate'], res['ave_reward'],
                                                                           res['ave_turns']))
    return res
...
def simulation_dqn():
    ...
    print("simulation success rate %s, ave reward %s, ave turns %s".format(
...
# <editor-fold desc="Functions about Discriminator">
def simulation_epoch_with_gan_control_filter(simulation_epoch_size, use_world_model=True,
    ...
    for episode in range(simulation_epoch_size):
        ...
        while (not episode_over):
            episode_over, reward = dialog_manager.next_turn(
                filter_experience_by_discriminator=filter_experience_by_discriminator)
            cumulative_reward += reward
            if episode_over:
                if reward > 0:
                    successes += 1
...
def run_episodes(count, status):
    ...
    for episode in range(count):
        ...
        if agt == 9 and params['trained_model_path'] == None:
            ...
            agent.predict_mode = False
            print("Simulation success rate %s, Ave reward %s, Ave turns %s, Best success rate %s".format(
        ...
        print("Progress: %s / %s, Success rate: %s / %s Avg reward: %.2f Avg turns: %.2f".format(
            episode + 1, count, successes, episode + 1, float(cumulative_reward) / (episode + 1),
            float(cumulative_turns) / (episode + 1)))
    print("Success rate: %s / %s Avg reward: %.2f Avg turns: %.2f".format(
