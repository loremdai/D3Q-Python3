...
class Discriminator(nn.Module):
    ...
    def train_single_batch(self, batch_size=16):
        ...
        for pos_exp, neg_exp in zip(pos_experiences, neg_experiences):
            loss += self.BCELoss(self.discriminate(pos_exp).view(-1,0), self.Variable(torch.ones(1))) + self.BCELoss(self.discriminate(neg_exp).view(-1,0), self.Variable(torch.zeros(1,1)))
