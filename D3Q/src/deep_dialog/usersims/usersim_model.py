...
class ModelBasedSimulator(UserSimulator):
    ...
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, start_set=None, params=None, discriminator=None):
        ...
        self.model = SimulatorModel(self.num_actions, self.hidden_size, self.state_dimension, self.num_actions_user, 1,
                                    nn_type=params['world_model_nn_type'], discriminator=discriminator)
    ...
    def train_iter(self, batch_size=1, num_batches=1):
        ...
        for iter_batch in range(num_batches):
            ...
            print(
                "total cost for user modeling %.4f, total loss for discriminator %.4f, total loss for generator %.4f, training replay pool %s" % (
                    float(self.total_loss) / (float(batch_size)),
                    float(self.total_loss_for_dis) / (float(len(self.training_examples)) / float(batch_size)),
                    self.total_loss_for_ge / (float(len(self.training_examples)) / float(batch_size)),
                    len(self.training_examples)))
